...
### Custom post types
As developer we are free to add any post type and tell Wordpress how to handle posts with that type. All post types are created at runtime and therefore need to be defined and described with PHP code in the theme or plugin. As theme developers we need to carefully plan the content of the websites and how it should be managed. A good practice is to not add more post types than is absolutely needed. Content variations can be made within one post type, so a new post type really demands for separation of functionality and content.
